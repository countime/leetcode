    ...
    class Solution {
        public List<String> subdomainVisits(String[] cpdomains) {
            ...
            for (String line : cpdomains) {
                ...
                String domain = split1[split1.length - 1];
                res.put(domain, res.getOrDefault(domain, 0) + count);
                for (int i = split1.length - 2; i >= 0; i--) {
                    domain = split1[i] + "." + domain;
                    res.put(domain, res.getOrDefault(domain, 0) + count);
                }
            }
...
//leetcode submit region end(Prohibit modification and deletion)
    ...
    public static void main(String[] args) {
        Solution s = new SubdomainVisitCount().new Solution();

        List<String> strings = s.subdomainVisits(new String[]{"1 a.b", "2 a"});
